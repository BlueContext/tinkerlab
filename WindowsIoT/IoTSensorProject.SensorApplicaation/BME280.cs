...
namespace IoTSensorProject.SensorApplicaation
{
    ...
    public class BME280Sensor
    {
        ...
        public async Task Initialize()
        {
            ...
            try
            {
                // Initiate the I2CConnectionSettings using the device address of the BME280
                I2cConnectionSettings settings = new I2cConnectionSettings(BME280_Address);

                // Set the I2C bus speed of the connection to fast mode
                ...
                // Instantiate the BME280 I2C device using the device id of the I2CBus and the I2CConnectionSettings
        ...
        public async Task<float> ReadTemperature()
        {
            ...
            byte txlsb = ReadByte((byte)eRegisters.BME280_REGISTER_TEMPDATA_XLSB);

            // Combine the the values into a 32-bit integer
            Int32 t = (tmsb << 12) + (tlsb << 4) + (txlsb >> 4);

            //Convert the raw value to the temp in degC
        ...
        public async Task<float> ReadPreasure()
        {
            ...
            // Read the temp first to load the t_fine value for compensation
            ...
            byte txlsb = ReadByte((byte)eRegisters.BME280_REGISTER_PRESSUREDATA_XLSB); // bits 7:
